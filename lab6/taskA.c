            ...
            for (int j = 0; j < grammar[i].prod_count; j++) {
                ...
                for (int k = 0; k < prod_len; k++) {
                    ...
                    if (is_non_terminal(current_sym)) {
                        ...
                        // If everything after it can be epsilon, or it's the last token
                        if (next_symbol) {
...
int main() {
    // Initialize sets with empty strings
    for (int i = 0; i < 26; i++) {
        first_sets[i][0] = '\0';
        follow_sets[i][0] = '\0';
    }

    // TASK A Grammar (expression grammar with left recursion removed):
    // E  -> T E'      (E)
    // E' -> + T E' | e   (A)
    // T  -> F T'      (T)
    // T' -> * F T' | e   (B)
    // F  -> ( E ) | i    (F)
    // Mapping: E' -> A, T' -> B, id -> i

    // Add rule for E: E -> T A
    grammar[rule_count].head = 'E';
    strcpy(grammar[rule_count].body[0], "TA");
    grammar[rule_count].prod_count = 1;
    rule_count++;

    // Add rule for A (E'): A -> + T A | e
